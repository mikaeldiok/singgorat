@php
    $status = \Modules\Feedback\Services\RemarkService::prepareStatusFilter();
    \Log::debug($status);
...
<script>
    ...
    $(document).ready(function(){
        $(document).on('click', 'button.choose-remark', function(e){
            var ele = $(this);  
            var fireAjax = false;
        ...
        function callPickRemark(ele){
                $.ajax({
                    type: "POST",
                    url: '{{route("frontend.bookings.pickRemark")}}',
                    data: {
                        "corporation_id" : "{{auth()->user()->corporation->id ?? 0}}",
                        "remark_id" : ele.attr("data-id"),
                        "status" : "{{$firstStat}}",
                        "_method":"POST",
                        "_token": "{{ csrf_token() }}"
                    },
